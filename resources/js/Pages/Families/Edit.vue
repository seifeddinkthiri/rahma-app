<template>
  <div>
    ...
    <trashed-message v-if="family.deleted_at" class="mb-6" @restore="restore">تم حذف هذا المنتفع . </trashed-message>
    <!--  -->
    <div class="bg-white rounded-md shadow overflow-hidden">
      <form @submit.prevent="update">
        ...
        <div class="flex items-center px-8 py-4 bg-gray-50 border-t border-gray-100">
          <button v-if="!family.deleted_at" class="inline-flex items-center justify-center ml-3 px-4 py-2 text-white text-sm font-medium bg-red-500 hover:bg-red-600 rounded focus:outline-none focus:ring-red-600 focus:ring-opacity-50" tabindex="-1" type="button" @click="destroy">حذف المنتفع</button>

          <loading-button :loading="form.processing" class="inline-flex items-center justify-center px-4 py-2 text-gray-700 text-sm font-medium bg-gray-200 hover:bg-gray-300 focus:bg-gray-300 rounded focus:outline-none" type="submit"> تعديل المنتفع </loading-button>
        </div>
      </form>
    </div>
    ...
    <div ref="members" class="bg-white rounded-md shadow overflow-hidden">
      ...
      <div class="flex flex-row items-center justify-around pr-4 w-full">
        <Link :href="`/members/${family.id}/create_new_one`" class="inline-flex items-center justify-center px-4 py-2 text-gray-700 text-sm font-medium bg-gray-200 hover:bg-gray-300 focus:bg-gray-300 rounded focus:outline-none">
          <span>إنشاء</span>
          <span class="hidden md:inline">&nbsp;الفرد</span>
        </Link>
        <p v-if="form.name == null" class="px-6 text-red-600">يجب تعيين معيل أسرة لهذه العائلة</p>
      </div>
        ...
        <table class="w-full whitespace-nowrap">
          ...
          <tbody>
            <tr v-for="member in family.members.data" :key="member.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
              ...
              <td class="border-t">
                <Link class="flex items-center px-6 py-4" :href="`/members/${member.id}/edit_caregiver`" tabindex="-1">
                  <div v-if="member.caregiver">
                    <ToggleCheckbox_caregiver :id="'sanitaion'" :class="'border-0'" :label="''" :isChecked="true" />
                  </div>
                  <div v-else>
                    <ToggleCheckbox_caregiver :id="'sanitaion'" :class="'border-0'" :label="''" :isChecked="false" />
                  </div>
                </Link>
              </td>
              ...
              <td class="w-px border-t">
                <Link class="flex items-center px-4" :href="`/members/${member.id}/show`" tabindex="-1">
    ...
    <div ref="members" class="bg-white rounded shadow overflow-hidden">
      ...
      <div class="flex items-center pr-4">
        <button v-if="family.home.length == 0" class="inline-flex items-center justify-center px-4 py-2 text-gray-700 text-sm font-medium bg-gray-200 hover:bg-gray-300 focus:bg-gray-300 rounded focus:outline-none" @click="openHomeModal">
          <span>إنشاء</span>
          <span class="hidden md:inline">&nbsp;المسكن</span>
        </button>
        <button v-else class="inline-flex items-center justify-center px-4 py-2 text-gray-700 text-sm font-medium bg-gray-200 hover:bg-gray-300 focus:bg-gray-300 rounded focus:outline-none" @click="edit_home">
          <span>تعديل</span>
          <span class="hidden md:inline">&nbsp;المسكن</span>
        </button>
      </div>
      ...
      <table class="w-full whitespace-nowrap">
        ...
        <tbody>
          <tr v-for="home in family.home" :key="home.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
            <td class="border-t">
            ...
            <td class="border-t"><div class="overflow-x-auto w-80 ...">
              <p class="truncate">
                {{ home.desciption }}
    ...
    <div class="mt-6 bg-white rounded shadow overflow-x-auto" ref="home_modal" v-if="show_home_modal">
      <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity"></div>
      <div class="fixed inset-0 flex items-center justify-center">
    ...
    <div ref="facilities" class="bg-white rounded shadow overflow-hidden">
      <br />
      <div class="flex items-center pr-4">
        <button class="inline-flex items-center justify-center px-4 py-2 text-gray-700 text-sm font-medium bg-gray-200 hover:bg-gray-300 focus:bg-gray-300 rounded focus:outline-none" @click="edit_facilities">
          <span>تعديل</span>
   ...
   <div ref="intervention_modal" v-if="show_intervention_modal">
    <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity"></div>
      <div class="fixed inset-0 flex items-center justify-center">
        <form @submit.prevent="store_intervention">
          <div class="w-96 h-auto bg-white rounded shadow-xl">
            <div class="flex flex-wrap -mb-8 -mr-6 p-8">
              <select-input v-model="intervention_form.type" class="pb-8 pr-6 w-full lg:w-1/2" :error="intervention_form.errors.type">
                <option hidden disabled :value="null">نوع التدخل</option>
                <option value="shipments">عيني</option>
                <option value="cash ">نقدي</option>
              </select-input>

              <text-input v-bind:class="['pb-8', 'pr-6', 'w-full', 'lg:w-1/2']" :id="intervention_form.type === 'shipments' ? 'shipments' : 'cash'" v-model="intervention_form.value" :error="intervention_form.errors.value" :placeholder="intervention_form.type === null ? 'القيمة' : intervention_form.type === 'shipments' ? 'الكمية' : 'المبلغ'" />
            ...
            <div class="flex items-center justify-end px-8 py-4 bg-gray-50 border-t border-gray-100">
              <button class="inline-flex items-center justify-center px-4 py-2 text-gray-700 text-sm font-medium bg-gray-200 hover:bg-gray-300 focus:bg-gray-300 rounded focus:outline-none" type="submit">إضافة</button>
              <button @click="show_intervention_modal = false" class="inline-flex items-center justify-center px-4 py-2 text-gray-700 text-sm font-medium bg-gray-200 hover:bg-gray-300 focus:bg-gray-300 rounded focus:outline-none" type="button">عودة</button>
            </div>
          </div>
        </form>
      </div>
    ...
    <div ref="members" class="bg-white rounded shadow overflow-hidden">
      <br />
      <div class="flex items-center pr-4">
        <button class="inline-flex items-center justify-center px-4 py-2 text-gray-700 text-sm font-medium bg-gray-200 hover:bg-gray-300 focus:bg-gray-300 rounded focus:outline-none" @click="openNoteModal">
          <span>إنشاء</span>
    ...
    <div class="mt-6 bg-white rounded shadow overflow-x-auto" ref="file_modal" v-if="show_file_modal">
      <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity"></div>
      <div class="fixed inset-0 flex items-center justify-center">
        <form @submit.prevent="save_file">
          <div class="w-96 h-auto bg-white rounded shadow-xl">
            <div class="p-6">
              <text-input id="title" v-model="files_form.title" :error="files_form.errors.title" placeholder="اكتب عنوان الملف " class="w-full" label="العنوان" />

              <file-input id="file" v-model="files_form.file" :error="files_form.errors.file" class="w-full" type="file" label="الملف" />
            </div>
            <div class="flex justify-end px-4 py-3 bg-gray-50">
              <button @click="show_file_modal = false" type="button" class="inline-flex items-center justify-center px-4 py-2 text-gray-700 text-sm font-medium bg-gray-200 hover:bg-gray-300 focus:bg-gray-300 rounded focus:outline-none">إلغاء</button>
    ...
    <div ref="files" class="bg-white rounded shadow overflow-hidden">
      <br />
      <div class="flex items-center pr-4">
        <button class="inline-flex items-center justify-center px-4 py-2 text-gray-700 text-sm font-medium bg-gray-200 hover:bg-gray-300 focus:bg-gray-300 rounded focus:outline-none" @click="show_file_modal = true">
          <span>إنشاء</span>
      ...
      <table class="w-full whitespace-nowrap">
        <tbody>
          <tr v-for="file in family.files" :key="file.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
            <td class="border-t">
              <button class="flex items-center px-6 py-4 focus:text-indigo-500" @click="edit_file(file.id)">
                <a :href="getFileUrl(file.file)">{{ file.title }}</a>
                <icon v-if="file.deleted_at" name="trash" class="flex-shrink-0 ml-2 w-3 h-3 fill-gray-400" />
              </button>
            </td>

            <td class="w-px border-t">
              <button @click="destroy_file(file.id)" type="button" class="flex items-center px-4" tabindex="-1">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                </svg>
              </button>
