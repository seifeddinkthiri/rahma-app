<template>
  <div>
    ...
    <div class="flex items-center justify-between mb-6">
      ...
      <button @click="store_family" class="inline-flex items-center justify-center px-4 py-2 text-gray-700 text-sm font-medium bg-gray-200 hover:bg-gray-300 focus:bg-gray-300 rounded focus:outline-none">
        <span>إنشاء</span>
      ...
      <table class="w-full">
        ...
        <tbody class="text-right">
          <tr v-for="family in families.data" :key="family.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
            ...
            <td class="border-t" v-if="family.name">
              <Link class="flex items-center mr-8 py-4" :href="`/families/${family.id}/edit`" tabindex="-1">
                <img v-if="family.photo" class="h-110 block -my-2 mr-2 w-10 rounded" :src="'uploads/' + family.photo" />
            ...
            <td class="relative border-t" colspan="5" v-if="family.members.length < 2 && family.name">
              <div class="flex items-center justify-between px-4 sm:px-20">
                ...
                <div class="flex items-center">
                  <div>
                    <Link class="underline" :href="`/members/${family.id}/create_new_one`" type="button"> إضافة </Link>
                  </div>
                  <div class="ml-4 mr-4">
                    <Link class="underline" :href="`/families/${family.id}/edit`"> تعديل </Link>
                  </div>
                  <div>
                    <button class="underline" type="button" @click="delete_family(family.id)">حذف</button>
                  </div>
                </div>
              </div>
            </td>
