<template>
  <div>
    ...
    <trashed-message v-if="user.deleted_at" class="mb-6" @restore="restore"> تم حذف هذا المستخدم </trashed-message>
    <div class="max-w-3xl bg-white rounded-md shadow overflow-hidden">
      <form @submit.prevent="update">
        ...
        <div class="flex items-center px-8 py-4 bg-gray-50 border-t border-gray-100">
          <button v-if="!user.deleted_at" class="text-red-600 hover:underline" tabindex="-1" type="button" @click="destroy">حذف المستخدم </button>
          <loading-button :loading="form.processing" class="btn-indigo ml-auto" type="submit">تحديث المستخدم</loading-button>
        </div>
      </form>
    </div>
  </div>
