<template>
  <div>
    ...
    <trashed-message v-if="deleted_family_id" class="mb-6" @restore="restore(deleted_family_id)">تم حذف هذا المنتفع . </trashed-message>
    <div v-if="show_modal" class="fixed z-50 inset-0 flex items-center justify-center rounded">
      <div class="fixed inset-0 bg-gray-500 bg-opacity-75 rounded transition-opacity"></div>
      <div class="z-10 w-96 max-h-screen bg-white rounded shadow-xl overflow-auto">
        <div class="px-4 py-6 bg-gray-50 rounded">
          <div class="grid gap-4 grid-cols-2 rounded">
            <button @click="store_family('elderly')" type="button" class="inline-flex flex-col items-center justify-center px-12 py-10 text-gray-700 text-sm font-medium bg-gray-200 hover:bg-gray-300 focus:bg-gray-300 rounded focus:outline-none">
              <img src="/svg-icons/aged.svg" width="140" height="35" />
              <p class="mt-4 text-base">مسن</p>
            </button>
            <button @click="store_family('widow')" type="button" class="inline-flex flex-col items-center justify-center px-12 py-10 text-gray-700 text-sm font-medium bg-gray-200 hover:bg-gray-300 focus:bg-gray-300 rounded focus:outline-none">
              <img src="/svg-icons/mother.svg" width="140" height="35" />
              <p class="mt-4 text-base">أرملة</p>
            </button>

            <button @click="store_family('divorced')" type="button" class="inline-flex flex-col items-center justify-center px-12 py-10 text-gray-700 text-sm font-medium bg-gray-200 hover:bg-gray-300 focus:bg-gray-300 rounded focus:outline-none">
              <img src="/svg-icons/woman.svg" width="140" height="35" alt="" class="logo_normal" />
              <p class="mt-4 text-base">مطلقة</p>
            </button>

            <button @click="store_family('single_mother')" type="button" class="inline-flex flex-col items-center justify-center px-12 py-10 text-gray-700 text-sm font-medium bg-gray-200 hover:bg-gray-300 focus:bg-gray-300 rounded focus:outline-none">
              <img src="/svg-icons/single_mother.svg" width="140" height="35" />
              <p class="mt-4 text-base">أم عزباء</p>
            </button>

            <button @click="store_family('family')" type="button" class="inline-flex flex-col items-center justify-center px-12 py-10 text-gray-700 text-sm font-medium bg-gray-200 hover:bg-gray-300 focus:bg-gray-300 rounded focus:outline-none">
              <img src="/svg-icons/family.svg" width="140" height="35" alt="" class="logo_normal" />
              <p class="text-md mt-4">عائلة معوزة</p>
            </button>
            <button @click="show_modal = false" type="button" class="inline-flex flex-col items-center justify-center px-12 py-10 text-gray-700 text-sm font-medium bg-gray-200 hover:bg-gray-300 focus:bg-gray-300 rounded focus:outline-none">إلغاء</button>
          </div>
        </div>
      </div>
    </div>
          ...
          <tr v-for="family in families.data" :key="family.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
            ...
            <td class="w-px border-t" v-if="family.name">
              <Link class="flex items-center px-4" :href="`/members/${family.id}/create_new_one`" tabindex="-1">
                <div v-if="family.is_family && family.members.length < 2 && family.name" class="flex items-center justify-between px-4 sm:px-20">
                  <p class="flex-grow px-3 py-2 text-red-600 whitespace-nowrap">تحتوي العائلة فردين على الأقل</p>
                </div>
              </Link>
            </td>
