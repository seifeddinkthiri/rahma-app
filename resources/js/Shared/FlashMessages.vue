<template>
  <div>
    <div
      ...
      class="flex items-center justify-between mb-8 max-w-xl bg-green-500 rounded"
    >
    ...
    <div
      ...
      "
      class="flex items-center justify-between mb-8 max-w-xl bg-red-500 rounded"
    >
      <div class="flex items-center">
        <svg
          class="flex-shrink-0 ml-4 mr-2 w-4 h-4 fill-white"
        ...
        <div v-else class="py-4 text-white text-sm font-medium">
          <span v-if="Object.keys($page.props.errors).length === 1"
            >هناك خطأ واحد في الإستمارة</span
          >
          <span v-else
            >هناك{{ Object.keys($page.props.errors).length }} أخطاء الإستمارة</span
          >
        </div>
      </div>
      <button type="button" class="group mr-2 p-2" @click="show = false">
